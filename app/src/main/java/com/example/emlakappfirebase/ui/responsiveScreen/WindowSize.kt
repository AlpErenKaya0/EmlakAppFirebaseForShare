...
//thanks to : https://www.youtube.com/watch?v=NPmgnGFzopA
...
fun rememberWindowSizeClass():WindowSizeClass{
    ...
    val windowWidthClass = when {
        ...
        width in 361..480 -> WindowSize.Medium(width)
        width in 481..720 -> WindowSize.Compact(width)
        else -> WindowSize.Large(width)

    }
    val windowHeightClass = when {
        height <= 360 -> WindowSize.Small(height)
        height in 361..480 -> WindowSize.Medium(height)
        height in 481..720 -> WindowSize.Compact(height)
